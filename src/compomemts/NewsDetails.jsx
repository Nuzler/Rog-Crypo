...
const NewsDetails = ({setIntro}) => {
    ...
    const cleanedHTML = cleanDescription(item.rawDesc);
    ...
    function cleanDescription(htmlString) {
      ...
      // Remove the first two images (if present)
      for (let i = 0; i < 2 && i < images.length; i++) {
        const img = images[i];
        if (img.parentElement) {
          img.parentElement.remove(); // also removes its <p> or container
        } else {
          img.remove();
        }
      }
    ...
    return (
      <div className='bg-black '>
        ...
        <div
      className="text-white font-Oleo-Script p-4 md:px-15 text-center items-center"
